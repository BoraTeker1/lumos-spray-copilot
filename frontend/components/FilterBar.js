...
export function FilterChip({ label, count, selected, onClick }) {
  return (
    <button
      ...
      className={`h-9 rounded-full px-3 text-xs font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${
        selected
    ...
    >
      {label}
      {count != null && (
        <span className={selected ? "ml-1 opacity-80" : "ml-1 text-gray-400"}>{count}</span>
      )}
    </button>
  );
}
