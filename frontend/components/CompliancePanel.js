...
// The compliance view — the "records requiring attention" table + harvest window,
// rendered as a tab of Evidence & compliance (formerly the standalone /compliance
// page; that route now redirects here). Every row is a REAL server-derived signal:
// a rule the engine triggered on a checked decision, or a decision whose
// documentation is still owed. No compliance math happens in the frontend.
// (The Pre-spray risk snapshot card renders on the farm-detail page only.)
...
export default function CompliancePanel() {
  const { activeFarm, refresh } = useFarmContext();
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        <div className="min-w-0 lg:col-span-8">
          <SectionCard
            ...
            icon={<ShieldAlert />}
            description="Rules the engine triggered on checked decisions, open conflicts, and documentation still owed — with the exact calculation and source authority. Signals come from entered records, not verified label data."
          >
            <FilterBar
