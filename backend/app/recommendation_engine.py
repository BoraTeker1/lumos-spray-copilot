...
# Appended verbatim to every pre-spray check so nobody mistakes user-entered PHI/REI
# values for label-verified ones.
...
def evaluate_planned_spray(
    ...
    Exactly three conservative checks, reusing the engine's thresholds:
    1. Repeated active ingredient — would this application exceed the recent-window limit?
    2. PHI arithmetic — intended date + user-entered PHI vs. expected harvest date.
    3. Explicitly linked scouting evidence — a recent observation whose normalized
       `visible_issue` exactly equals the normalized target. No substring or semantic
       matching: anything short of an exact normalized match counts as "no explicitly
       linked scouting observation", which is all the flag claims.

    Parameters
    ----------
    planned: object with `intended_date`, `product_name`, `active_ingredient`,
        `target_pest_or_disease`, `pre_harvest_interval_days`.
    Other parameters as in `generate_recommendation`.
    """
    if today is None:
        today = date.today()
    ...
    # --- Check 1: repeated active ingredient (counting the planned application) --------
    overuse = False
    ...
    if planned_ai:
        ...
        if prior_uses + 1 > SAME_INGREDIENT_MAX:
            score += 2
            overuse = True
            result.flags.append(
    ...
    # --- Check 2: intended date + user-entered PHI vs. expected harvest ---------------
    phi_risk = False
    harvest = getattr(farm, "expected_harvest_date", None)
    ...
    if harvest is not None and phi:
        ...
        if harvest < phi_clears_on:
            ...
            phi_risk = True
            result.flags.append(
                f"Pre-harvest interval risk: applying '{product}' on {intended.isoformat()} "
                f"with the entered PHI of {phi} days would clear on {phi_clears_on.isoformat()}, "
                f"after the expected harvest on {harvest.isoformat()}. Residue risk appears "
                f"elevated — review harvest timing with your agronomist before applying."
            )

    # --- Check 3: explicitly linked scouting evidence for the target ------------------
    target = (getattr(planned, "target_pest_or_disease", None) or "").strip().lower()
    recent_obs = _recent(scout_observations, "observation_date", today)
    linked_obs = [
        o for o in recent_obs
        if target and (getattr(o, "visible_issue", None) or "").strip().lower() == target
    ]
    scouting_evidence_linked = bool(linked_obs)
    if not scouting_evidence_linked:
        score += 1
        target_label = f"'{target}'" if target else "the stated target"
        result.flags.append(
            f"No scouting observation explicitly referencing {target_label} was found in the "
            f"last {RECENT_WINDOW_DAYS} days — consider inspecting first to confirm pressure "
            f"before applying."
        )

    # --- Map score to the same cautious risk levels ------------------------------------
    if score >= 3:
        result.risk_level = RISK_ELEVATED
    elif score >= 1:
        result.risk_level = RISK_MODERATE
    ...
    result.next_action = _derive_next_action(
        ...
        high_sev=False,
        rei_risk=False,
        has_recent_scouting=scouting_evidence_linked,
    )

    result.signals = {
        "phi_risk": phi_risk,
        "repeated_active_ingredient_risk": overuse,
        "scouting_evidence_linked": scouting_evidence_linked,
    }

    if not result.flags:
        result.flags.append(
            "No elevated risk detected for this planned application from current records. "
            "The decision remains with you and your agronomist."
        )

    result.recommendation_text = (
        _format_text(result.risk_level, result.next_action, result.flags)
        + "\n"
        + PLANNED_SPRAY_DISCLAIMER
    )
    return result
