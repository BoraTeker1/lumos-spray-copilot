...
def test_request_is_never_an_approval_state_walk(client):
    ...
    assert _financing_state(client, plan["id"]) == "offer_accepted"
...
def test_decisions_are_one_shot(client):
    ...
    offer = _enter_offer(client, quote["id"])
    assert client.post(
        f"/financing-offers/{offer['id']}/decision", json={"action": "accepted"}
    ).status_code == 200
...
def test_only_one_accepted_offer_per_plan(client):
    farm = _farm(client)
    plan, quote = _quoted_plan(client, farm["id"])
    first = _enter_offer(client, quote["id"])
    second = _enter_offer(client, quote["id"], provider_name="Other Lender")
    client.post(f"/financing-offers/{first['id']}/decision", json={"action": "accepted"})
    resp = client.post(
        f"/financing-offers/{second['id']}/decision", json={"action": "accepted"}
    )
    assert resp.status_code == 409
    assert "already accepted" in resp.json()["detail"]


def test_expired_offer_cannot_be_accepted(client):
    farm = _farm(client)
    plan, quote = _quoted_plan(client, farm["id"])
